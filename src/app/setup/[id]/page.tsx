      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge className="bg-[#e10600] uppercase">{typeLabel(setup.type)}</Badge>
          ...
          <div className="mt-6 flex flex-wrap gap-5 text-sm text-muted-foreground">
            <span className="inline-flex items-center gap-1.5">
              <Flag className="h-4 w-4 text-[#e10600]" /> {setup.track.country}
            </span>
            {setup.track.laps > 0 && (
              <span className="inline-flex items-center gap-1.5">
                <Gauge className="h-4 w-4 text-[#e10600]" /> {setup.track.laps} кругов ·{' '}
                {setup.track.lengthKm.toFixed(3)} км
              </span>
            )}
            <span className="inline-flex items-center gap-1.5">
              <TrendingUp className="h-4 w-4 text-[#e10600]" /> {setup.sales} продаж
            </span>
          </div>
        ...
        <div className="lg:sticky lg:top-24 lg:h-fit">
          ...
          <Card className="mt-4 border-border/70 bg-card/70 p-5 text-sm text-muted-foreground">
            <div className="flex items-center gap-2 font-semibold text-foreground">
              <ShieldCheck className="h-4 w-4 text-[#e10600]" /> Что вы получаете
            </div>
